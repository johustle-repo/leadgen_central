...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request): Response
    {
        ...
        // Reply data is a Super Administrator-only feature; other roles get
        // nothing here rather than a zeroed-out figure.
        if ($user->isSuperAdministrator()) {
            $replies = EmailReply::query()->when(! $all, fn ($query) => $query->whereBelongsTo($user, 'agent'));
            $stats['unread_replies'] = (clone $replies)->where('is_read', false)->count();
            $stats['possible_reply_leads'] = (clone $replies)->whereIn('classification', ['interested', 'possible_lead'])->count();
        }
